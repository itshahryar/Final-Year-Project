...
# Set paths
input_dir = "Random6"  # Folder containing multiple videos
...
collection_alerts = db["detectedanomalies"]
incidents_collection = db["anomalyresponses"]
...
def fetch_ppe_requirements(video_name):
    ...
    if "SafetyVests" in REQUIRED_PPE:
        REQUIRED_PPE.remove("SafetyVests")
        REQUIRED_PPE.append("Safety Vest")

    if "vest" in REQUIRED_PPE:
        REQUIRED_PPE.remove("vest")
        REQUIRED_PPE.append("Safety Vest")   

# Replace "helmet" with "Hardhat"
    if "helmet" in REQUIRED_PPE:
        REQUIRED_PPE.remove("helmet")
        REQUIRED_PPE.append("Hardhat")

    print (REQUIRED_PPE, "for",video_name)
...
def save_alert_to_db(violation_type, video_name):
    ...
    print(f"✅ Logged in DB: {anomaly_id}")
                ...
                # 📝 Create Incident (Status: Unresolved)
    incident_record = {
